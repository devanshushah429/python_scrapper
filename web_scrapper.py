...
def process_page(selenium_web_page_scrapper:SeleniumWebPageScrapper, current_page_url, xpaths):

    mongo_db_handler = MongoDBHandler(xpaths["connection_string"], xpaths["database_name"], xpaths["collection_name"])
    selenium_web_page_scrapper.load_url(current_page_url)
    listing_page_handler = ListingPageHandler(current_page_url, xpaths)
    try:
        next_page_url, blocks_data = listing_page_handler.fetch_block_data_selenium_chrome_driver(selenium_web_page_scrapper)
        all_data = []

        for block_data in blocks_data:
            details_page_handler = DetailPageHandler(block_data.get("url"), xpaths)
            details_page_data = details_page_handler.scrape_details_page_using_selenium_chromedriver(selenium_web_page_scrapper)
            data = {
